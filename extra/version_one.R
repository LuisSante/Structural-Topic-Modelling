...
if(!is.null(stmFit)) {
  ...
  plot(stmFit, type="labels", topics=c(2, 4, 6))
  dev.off()
  
  # Graficar perspectivas entre temas
  pdf("stm-plot-prevfit-perspectives.pdf", width=14, height=12.5)
  plot(stmFit, type="perspectives", topics=c(4,6))
  dev.off()
  
  # Realizar búsqueda del mejor número de temas (searchK)
  kResult <- searchK(docs, vocab, K=c(2, 4, 6), prevalence=~s(at), data=meta)
  ...
  # Etiquetar temas
  labelTopicsSel <- labelTopics(stmFit, c(2, 4, 6))
  ...
  thoughts7 <- findThoughts(stmFit, texts=meta$reviews, n=3, topics=6)$docs[[1]]
  pdf("stm-plot-thoughts7.pdf", width=10, height=8.5)
  plotQuote(thoughts7, width=40, main="Topic 6")
  dev.off()
  
  # Estimar efectos de las covariables
  prep <- estimateEffect(1:6 ~ s(at), stmFit, meta=meta, uncertainty="Global")
  pdf("stm-plot-estimate-effect.pdf", width=10, height=8.5)
  plot(prep, covariate="at", topics=c(2, 4, 6), model=stmFit, method="continuous",
